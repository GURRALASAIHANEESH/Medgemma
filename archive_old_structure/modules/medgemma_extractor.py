"""
MedGemma Clinical Reasoning Engine - CORRECTED INTERFACE
NO chat templates, NO 8-bit quantization, PLAIN causal LM usage
"""
import torch
...
OFFLOAD_DIR = Path("D:/medgemma_offload")
...
class MedGemmaClinicalReasoner:
    ...
    def load_model(self):
        """Load MedGemma with stable FP16 configuration"""
        try:
            ...
            # Load tokenizer - NO CHAT TEMPLATE
            ...
            # Load model - STABLE FP16, NO QUANTIZATION
            # Load model - CPU ONLY, STABLE
            self.model = AutoModelForCausalLM.from_pretrained(
                str(model_path),
                local_files_only=True,
                trust_remote_code=True,
                torch_dtype=torch.bfloat16,  # Half precision for RAM efficiency
                device_map="cpu",
                low_cpu_mem_usage=True,
                offload_state_dict=True  # Critical for Windows RAM management
            )
    ...
    def reason_about_case(self, structured_data: dict) -> dict:
        """
        Clinical reasoning on structured data - PLAIN CAUSAL LM INFERENCE
        """
        try:
            # Build plain text prompt (NO CHAT TEMPLATE)
            prompt = self._build_clinical_prompt(structured_data)
            
            # Tokenize - DIRECT, NO CHAT FORMATTING
            ...
            # Move to model device - USE model.device, NOT hardcoded GPU 0
            ...
            return self._parse_reasoning_output(response, structured_data)
    ...
    def _parse_reasoning_output(self, response: str, original_data: dict) -> dict:
        """Parse model response into structured format with clinical entity recognition"""
        ...
        if original_data.get('bp_systolic'):
            ...
            if bp_sys >= 160:  # Severe hypertension
                risk = 'HIGH'
                referral_urgent = True
                safety_net_triggered = True
                logger.warning(f"Safety net: Severe HTN BP={bp_sys} mmHg → HIGH risk")
            elif bp_sys >= 150:  # Stage 2 hypertension
                if risk in ['LOW', 'UNKNOWN']:
                    risk = 'HIGH'
                    referral_urgent = True
                    safety_net_triggered = True
...
def get_clinical_reasoner() -> MedGemmaClinicalReasoner:
    """Get singleton MedGemma reasoner"""
...
def clinical_reasoning(structured_data: dict) -> dict:
