    ...
    def combine_with_symptoms(self, 
        ...
        if lab_risk in ["MODERATE", "HIGH"]:
            if "bp" in (lab_reason or "").lower() or "hypertension" in (lab_reason or "").lower():
                if has_neuro:
                    neuro_list = ', '.join(categories.get('neurological', []))
        ...
        if lab_risk == "HIGH":
            symptom_list = ', '.join(present_symptoms[:3])
        ...
        if lab_risk == "MODERATE" and has_edema:
            edema_list = ', '.join(categories.get('edema', []))
        ...
        referral = lab_risk == "HIGH"
        symptom_note = f" with {len(present_symptoms)} symptom(s)" if present_symptoms else ""
        return lab_risk, f"{lab_reason}{symptom_note}", referral
