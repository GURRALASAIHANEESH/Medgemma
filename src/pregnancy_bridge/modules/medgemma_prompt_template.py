...
CONTEXT_INTERPRETER_PROMPT = """ROLE: medical explanation generator (support only).
INPUT:
...
TASK:
1) Produce exactly 3 short bullet lines:
  a. why the evidence matters clinically,
  b. why escalation occurred now,
  c. what complication(s) this aims to prevent.
2) MUST include at least one exact string from evidence_summary verbatim.
3) Do NOT change risk_category or recommend actions beyond explanation.
4) Be concise, clinical, and targeted to a clinician reviewer.
